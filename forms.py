...
def build_form_fields(form, variables):
    ...
    sections = config.forms[form]["sections"]
    ...
    for section in sections:
        ...
        for field in section["fields"]:
            ...
            fielddescription = config.fields[field]
...
def retrieve_variable(self, field_name):
    ...
    if config.error_conditions.has_key(field_name):
        ...
        if self.request.get(field_name):
            ...
            if config.error_conditions[field_name].has_key("not_numeric"):
                try:
                    # cast the variable to a variable
                    variable = float(variable)

                    if config.fields[field_name].has_key("max") and variable > config.fields[field_name]["max"]:
                        errors.append("too_high")
                    if config.fields[field_name].has_key("min") and variable < config.fields[field_name]["min"]:
            ...
            if config.error_conditions[field_name].has_key("not_present"):
                # add the key for the error to the errors array
...
def build_variable_set(self, form):
    ...
    for section in config.forms[form]["sections"]:
